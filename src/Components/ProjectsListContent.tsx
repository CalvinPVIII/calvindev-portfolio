  ...
  return (
    <div id="projects-list-container">
      {Object.values(projects).map((project, index) => (
        <motion.div className="project-item" onClick={() => handleProjectClick(project.name)} key={project.name}>
          ...
          {Object.values(projects).indexOf(project) !== Object.values(projects).length - 1 ? (
            <motion.div className="project-list-spacer" initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.5 }}></motion.div>
          ) : null}
        </motion.div>
      ))}
    </div>
